...
class AgenticWorkflowDashboardDemo:
    ...
    def get_available_brands(self):
        """Get demo brands from demo_data folder"""
...
def main():
    ...
    <div class="demo-notice">
        ...
        <p>Featuring real social media analysis data for selected brands.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not available_brands:
        st.error("No brands with analysis data found in demo_data/")
        return
    ...
    # Brand info
    st.sidebar.markdown("## 📋 Demo Brands")
    st.sidebar.markdown(f"**{len(available_brands)} brands available:**")
    for brand in available_brands[:6]:  # Show first 6
        st.sidebar.markdown(f"• {brand}")
    if len(available_brands) > 6:
        st.sidebar.markdown(f"• ... and {len(available_brands) - 6} more")
